...
public class WebMessagesPage {
    ...
    public static WebMessagesPage of(Page<com.microel.microelhub.storage.entity.Message> page){
        ...
        webMessagesPage.content = page.getContent().stream().map(m->new Message(m.getChatMsgId(), UUID.fromString(m.getChat().getUser().getUserId()), m.getText(), m.getOperatorMsg())).collect(Collectors.toList());
